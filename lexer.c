...
//running count of line and character, so as to populate the fields in
the tokenInfo structure in getNextToken(twinBuffer B)


void removeComments(char *testcaseFile, char *cleanFile){//assumes char
array terminates with \0, and returns a char array terminated with \0 as
well
     FILE* tcf=fopen(testcaseFile,"r");
     FILE*  cf=fopen(cleanFile,"w");

     if(tcf==NULL||cf==NULL){
         printf("Error: file %s or %s not found in
directory",testcaseFile,cleanFile);
     }
...
FILE *getStream(twinBuffer* buffer, FILE *fp, int bufno) {//populates
twin buffer(a global variable) with 200 characters
     int charsread;
...
tokenInfo int_token(twinBuffer* B, int len){
     ...
     /*if(len>30){
         fprintf(stderr,"Line %d: Integer length limit exceeded.
Truncated to lowest 30 digits",B->lno);
         i=(i+len-30)%60;
     }*/
...
tokenInfo float_token(twinBuffer* B, int len){
     ...
     /*if(len>30){
         fprintf(stderr,"Line %d: Real number length limit exceeded.
Integral part truncated to lowest 30 digits",B->lno);
         i=(i+len-30)%60;
     }*/
...
if((c<=8)&&(c>0)||((c>=11)&&(c<=31))||c==27||c==34||c==36||c==63||c==92||c==94||c==96||((c>=123)&&(c<=125))||c>125){return
1;}
             else return 0;
}
...
//NEED ERROR FUNCTION for incorrect pattern to check if the other flags
are there first and work only if they're off
...
tokenInfo getNextToken(twinBuffer* B, FILE* fp, lookuptbl* table){
     ...
     while (contflag){
         ...
         currchar=B->buf[(B->index+len)%60];//character to examine- len
characters of the token have already been examined
...
if((48<=currchar)&&(57>=currchar)){state=11;break;}
             ...
             case 69://nice
                 if(currchar==' '){state=69;break;}
                 len--;//we need a retract here so as not to see the
next char
                 B->index=(B->index+len)%60;
...
//0) One mandatory retract in all final states, so that B->index now
points to the last character seen to get to current state and not the
next character to examine
//1) retract again if needed (reduce B->index, changing currbuf and
B->index as needed if the new value is negative)
//2) set parameters of token to return (by calling real_token,
int_token, lookup and insert or set_lexeme) / print error
//3) set both pointers= (B->index+1)%30 (change currbuf if necessary) if
returning token (setting startptr=B->index actually not needed; next
call to getNextToken will do this for us anyway
//4) inc line number if applicable
//5) return token/set state to 0

//The setting of the forward and start pointer to the required value is
taken care of automatically by the increment at the end of the loop and
the setting
//of startpointer=B->index in state 0, in the case where we go to state
0 instead of returning token. Otherwise, it must be done before the
return statement.

//installing token/looking up a token in the symbol table could be
handled by the invoking parser function.
//it should know to do this since insertion/lookup is needed if and only
if an identifier token is returned.
//Or we could make a symbol table structure here, initialize it in the
parser function and pass it into getNextToken().
...
strcpy(table[17].keyw,"parameters");table[17].tkid=TK_PARAMETERS;
     ...
     if(tkinf.err==0)printf("Line no. %d    Lexeme %s   Token
%s\n",tkinf.lno,tkinf.strlex,terminalNames[tkinf.tkid]);
     else if(tkinf.err==3)
         {
          if(tkinf.tkid==TK_NUM){
           printf("Line No %d : Error :Integer is longer than the
maximum length of 30 digits\n",tkinf.lno);
          }
          else if(tkinf.tkid==TK_RNUM){
           printf("Line No %d : Error :Floating-point number is longer
than the maximum length of 30 characters\n",tkinf.lno);
          }
          else if(tkinf.tkid==TK_ID){
           printf("Line No %d : Error :Variable is longer than the
prescribed length of 20 characters\n",tkinf.lno);
          }
          else{
           printf("Line No %d : Error :Record, union, field or function
identifier is longer than the maximum length of 30 characters\n",tkinf.lno);
          }
         }
     else if(tkinf.err==2)printf("Line no: %d : Error: Unknown pattern
<%s>\n",tkinf.lno,tkinf.strlex);
     else printf("Line No %d : Error: Unknown Symbol
<%s>\n",tkinf.lno,tkinf.strlex);
     }
     //printf("Buffer: %s\n", buffer->buf);
     //printf("Index: %d\n", buffer->index);
     ...
     else if(tkinf.tkid==TK_REAL)printf("value of tkn is
%f\n",tkinf.val.rval);
